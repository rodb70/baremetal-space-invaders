...
volatile uint32_t* frame_buffer_init(void)
{
    ...
    m->window = SDL_CreateWindow( "space-invaders", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                  WIDTH*3, HEIGHT*3, 0 );
    assert( m->window );
...
void fillScreenDMA(volatile unsigned short color)
{
...
{
    drawPortionImage(x,y, width, height, image );
}
